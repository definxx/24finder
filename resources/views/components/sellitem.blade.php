            ...
            <form method="POST" class="bg-white p-6 rounded-lg shadow-lg" action="{{ route('items.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center border border-gray-300 rounded-md">
                        ...
                        <input name="description" class="w-full p-2 outline-none" type="text" id="description" placeholder="Enter a brief description"  />
                    </div>
                    @error('description')
                        <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                    @enderror
...
<div class="mb-4">
    <label class="block text-gray-700 text-sm font-bold mb-2" for="images">
        Upload up to 5 images in JPG, PNG, JPEG, or WebP format. Each image must not exceed 4MB.
    </label>
